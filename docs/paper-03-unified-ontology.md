...
abstract: |
  The standard mathematical ontology of the continuum — the real numbers ℝ with their
  ...
  (ultrametric completions of ℚ). Depth is physically real — it is required for dynamics,
  ...
  continuum is ℝ_c × ∏_p ℚ_p^c — the computable reals together with the computable
  p-adic numbers at finitely many primes. Ostrowski's theorem is the deep organizing
  principle: there are no other completions of ℚ, so there are no other physically real
...
The completions of ℚ, by Ostrowski's theorem, partition into:

| Axis | Mathematical Object | Physical Status | Key Property |
|:-----|:--------------------|:----------------|:-------------|
| **Depth** | Archimedean completion ℝ, computable subset ℝ_c | **Physically real** | Connectedness, dynamics, causality, limits |
| **Breadth** | ℝ \ ℝ_c — the non-computable reals | **Physically vacuous** | Unfalsifiable, no measurement-distinguishable predictions |
| **Valuation** | ℚ_p for primes p, computable subset ℚ_p^c | **Physically real** (as information carriers) | Discrete quantum numbers, spin, hierarchical information structure |
...
# 2. Depth: The Archimedean Completion ℝ_c

## 2.1 What It Is

The Archimedean completion of ℚ is the unique connected, ordered, complete (in the Cauchy sense) field containing ℚ. Its computable subset ℝ_c preserves all physically relevant properties:

| Property | Holds in ℝ_c? | Physical Role |
|:---------|:---:|:--------------|
| Ordered field | ✓ | Basic arithmetic of physical quantities |
| Computable completeness | ✓ | All measurement sequences with computable error bounds converge |
| Connectedness | ✓ | Causality, topology, intermediate value theorem |
| Real closure | ✓ | Algebraic closure of physically real numbers |
| Uncountability | ✗ | Not required — |ℝ_c| = ℵ₀ but this is meta-theoretic |
...
**Depth is the minimal completion of ℚ required for dynamics.** And ℝ_c — not ℝ — is that completion.

## 2.3 What It Does NOT Require

Depth does not require:

- Uncountability (|ℝ_c| = ℵ₀)
- The Continuum Hypothesis (distinction between ℵ₁ and ℵ₂ is physically inert)
...
- Chaitin's Ω (its binary expansion can never be measured; it is a proof-theoretic artifact)

---

# 3. Breadth: The Dark Continuum ℝ \ ℝ_c

## 3.1 What It Is

The complement ℝ \ ℝ_c consists of all non-computable real numbers — reals for which there is no Turing machine that approximates them with a computable modulus of convergence. This set has cardinality 2^{ℵ₀} (continuum).
...
The proof is constructive: any measurement protocol defines a computable functional, and computable functionals cannot distinguish between reals that agree on computable tests. The excess reals in ℝ \ ℝ_c are physical duplicates of their computable shadows — the nearest computable rational approximations.
...
This is not a rejection of classical mathematics. ZFC proves true things about ℝ that are physically vacuous. The distinction between ℝ and ℝ_c is the distinction between a mathematical universe that includes the full power set of ℕ and one that includes only the computable subsets. The latter is sufficient for physics; the former is richer but physically inert.

The Continuum Hypothesis, the Banach-Tarski paradox, Chaitin's Ω — these are true (or independent) in ZFC but have zero physical content. They quantify over objects — non-computable subsets, non-measurable partitions, non-computable reals — that are not physically accessible.

---

# 4. Valuation: The Ultrametric Completions ℚ_p

## 4.1 What They Are

For each prime p, the p-adic numbers ℚ_p form a complete, ultrametric field containing ℚ. Unlike ℝ, they are totally disconnected — their topology is a Cantor set. Their geometry is tree-structured: the Bruhat-Tits building for SL_n(ℚ_p) is a simplicial complex encoding the lattice-theoretic structure of the completion.

The computable subset ℚ_p^c (p-adic numbers approximable by Turing machines) is countable and sufficient for physical content — paralleling the ℝ_c/ℝ distinction.
...
|:--|:--------------|:------------|
| 2 | Spin, fermion/boson statistics | ℤ₂ grading on Bruhat-Tits tree boundary |
| 3 | Color charge (QCD) | Vertex labels in SL₃(ℚ₃) Bruhat-Tits building |
...
A natural objection: each ℚ_p has cardinality 2^{ℵ₀} (continuum), just like ℝ. Doesn't the breadth problem apply to p-adic numbers too?

**No. The distinction is operational:**

- Non-computable reals (ℝ \ ℝ_c) are physically indistinguishable from each other and from their computable shadows. They add no measurement-distinguishable content (Paper I, Theorem 4.3).

- Non-computable p-adic numbers (ℚ_p \ ℚ_p^c) face the same objection — they too are physically vacuous. The computable p-adic numbers ℚ_p^c are sufficient for physical content.

- **But the computable p-adic numbers make falsifiable physical predictions** — Gromov δ = 0 for ZBW transitions, ℤ₂ invariant distinguishing Dirac from Majorana, p-adic valuation gaps between optimal and random codes. These predictions are testable independent of any non-computable structure.

Valuation is physically real (through ℚ_p^c) because it generates measurement-distinguishable predictions. Breadth is vacuous regardless of which completion it appears in.
...
|:-----|:---:|:---:|:----------------|
| **Depth (ℝ_c)** | ✓ — all x ∈ ℝ_c are computable by definition | ✓ — different computable reals produce different measurement sequences | Physically real |
| **Breadth (ℝ \ ℝ_c)** | ✗ — no finite procedure approximates them | ✗ — they are unfalsifiable (Theorem 4.3, Paper I) | Physically vacuous |
| **Valuation (ℚ_p^c)** | ✓ — computable p-adic numbers are Turing-approximable | ✓ — falsifiable predictions (Gromov δ, ℤ₂ invariant, valuation gaps) | Physically real |
| **Valuation breadth (ℚ_p \ ℚ_p^c)** | ✗ — same as breadth above | ✗ — same as breadth above | Physically vacuous |
...
𝔸_ℚ^c — the ring of computable adeles — is the candidate framework for a unified adelic physical theory: a product of the computable reals and all computable p-adic numbers, restricted to finitely many non-Archimedean places for any given physical system.
...
2. **The Continuum Hypothesis cannot affect any measurement outcome.** Any theory whose predictions change depending on whether |ℝ| = ℵ₁ or |ℝ| > ℵ₁ is not a physical theory — it is a mathematical artifact.
...
2. **ℤ₂ invariant distinguishes Dirac (invariant = +1) from Majorana (invariant = -1).** This is a momentum-dependent diagnostic at the field-theoretic level — testable in condensed matter systems hosting Majorana zero modes.
...
1. **No Archimedean perturbation can affect p-adic fixed points.** The mutual singularity of ℝ and ℚ_p topologies (Ostrowski) implies that Majorana zero modes on Bruhat-Tits trees are immune to thermal, electromagnetic, and vibrational decoherence — all of which are Archimedean perturbations.

2. **Physics is finite-dimensional over ℚ at finitely many places.** The number of physically relevant primes S is finite. This constrains the possible structure of beyond-Standard-Model theories: they may add new primes but not a continuous infinity of them.
...
- **Physical:** computable reals (ℝ_c), computable p-adic numbers at finitely many primes (ℚ_p^c), and their finite products.
- **Mathematical but physically vacuous:** the full power set of ℕ, non-computable reals, the Continuum Hypothesis, and other set-theoretic structures that quantify over physically inaccessible objects.
...
1. **Quantum field theory should be reformulated over the ring of computable adeles A_ℚ^c, not over ℝ⁴.** The familiar QFT over ℝ^{3,1} is the Archimedean projection of a richer adelic theory.
...
The physical continuum is not ℝ. It is ℝ_c together with ℚ_p^c at finitely many relevant primes. Depth stays. Breadth goes. Valuation enters — not as geometry, but as information.

The OC program's ultimate claim: **all physically real entities are generated from finite D/R procedures over ℚ, completed at finitely many places.** The dark continuum — uncountability-as-breadth — is a mathematical ghost. The connected continuum — uncountability-as-depth — is physically real but countably realized. And the p-adic dimensions — the third axis the 2D model missed — are the missing link between quantum discreteness and number-theoretic structure.
